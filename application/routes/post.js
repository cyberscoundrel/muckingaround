...
router.post('/', uploader.single("isub"), (req, res, next) => {
    ...
    sharp(filepath).resize(200).toFile(destination).then(() => {
        PostModel.create(title,filepath,filethumb,req.session.userid,desc,req.session.username)
        .then((result) => 
        {
            if(result)
            {
            ...
            {
                throw new PostError('coult not create post','/post',500);
            }
        })
        .catch((err) => {
...
router.get('/posts/:id', (req, res, next) => {
    ...
    .then((results) => {
        if(results)
        {
            res.render('partials/imagepost',{script:'../../post.js', post_title:results.post_title, username: results.username, image_name: results.image_name, image_name: results.create, image:'../../' + results.media, description: results.description});
        }
        else
        {
